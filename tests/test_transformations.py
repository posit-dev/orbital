...
def test_apply_post_transform_invalid_transform():
    ...
    with pytest.raises(NotImplementedError, match="Post transform 'INVALID' is not implemented"):
...
def test_logistic_transform_computes_correct_values():
    ...
    assert abs(computed_value - expected_value) < 1e-10, f"LOGISTIC: expected {expected_value}, got {computed_value}"
...
def test_none_transform_computes_correct_values():
    ...
    assert computed_none == test_value, f"NONE: expected {test_value}, got {computed_none}"
...
def test_softmax_transform_computes_correct_values():
    ...
    assert computed_value == 1.0, f"SOFTMAX single value: expected 1.0, got {computed_value}"
    ...
    for i, (name, expected) in enumerate(zip(["class_0", "class_1", "class_2"], expected_softmax)):
    ...
    assert abs(total_sum - 1.0) < 1e-10, f"SOFTMAX values should sum to 1.0, got {total_sum}"
